...
export function SubAdminOwnPolicies({ policies = [], onRefresh, axiosInstance }) {
  const navigate = useNavigate();
  const [query, setQuery] = useState("");

  // Nothing is deleted until the modal's Confirm Delete is pressed. The server
  // additionally enforces that a Sub Admin may only delete their own policies.
      ...
      <div className="bg-[#0d0f1d] border border-[#1e2238] rounded-2xl p-6 h-fit shadow-xl">
        <div className="flex items-center justify-between gap-2 mb-4">
          <div className="flex items-center gap-2">
            <div className="p-2.5 bg-white/5 border border-white/5 rounded-xl text-[#00f0ff] font-bold">
              <FileText size={16} />
            </div>
        ...
        <div className="mt-4 text-[11px] text-[#6b7280]">
          Showing policies in your agent scope.
        </div>
      </div>
