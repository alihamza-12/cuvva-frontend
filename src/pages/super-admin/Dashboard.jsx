...
const AccountManagementView = ({ onActionSuccess }) => {
  ...
  const toggleAccountStatus = async (userId, currentStatus) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/management/status/${userId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  if (loading) return <div className="text-xs text-[#6b7280] animate-pulse">Synchronizing access directories...</div>;
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="bg-[#0d0f1d] border border-[#1e2238] rounded-2xl p-6">
        <h3 className="flex items-center gap-2 mb-4 text-xs font-bold tracking-wider text-white uppercase">
          <ShieldAlert size={14} className="text-[#644aff]" /> Sub-Admin System Operators
        </h3>
            ...
            <tbody className="divide-y divide-[#1e2238]/50 text-slate-300">
              {subAdmins.map((admin) => (
                <tr key={admin._id} className="hover:bg-white/[0.01]">
                  <td className="py-3 font-semibold text-white">{admin.fullName}</td>
                  <td className="py-3 text-[#8a8fbc]">{admin.email}</td>
                  <td className="py-3">
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                      admin.status === "Active" ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"
                    }`}>
                      {admin.status || "Active"}
                    </span>
                  </td>
                  <td className="py-3 text-right">
                    <button
                      onClick={() => toggleAccountStatus(admin._id, admin.status || "Active")}
                      className="transition-colors text-slate-400 hover:text-white"
                    >
                      {(admin.status || "Active") === "Active" ? <ToggleRight size={20} className="inline text-green-400" /> : <ToggleLeft size={20} className="inline text-red-400" />}
                    </button>
                  </td>
                </tr>
              ))}
      ...
      <div className="bg-[#0d0f1d] border border-[#1e2238] rounded-2xl p-6">
        <h3 className="flex items-center gap-2 mb-4 text-xs font-bold tracking-wider text-white uppercase">
          <Users size={14} className="text-[#644aff]" /> Registered Platform Customers
        </h3>
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-left border-collapse">
            <thead>
            ...
            <tbody className="divide-y divide-[#1e2238]/50 text-slate-300">
              {customers.map((customer) => (
                <tr key={customer._id} className="hover:bg-white/[0.01]">
                  <td className="py-3 font-semibold text-white">{customer.fullName}</td>
                  <td className="py-3 text-[#8a8fbc]">{customer.email}</td>
                  <td className="py-3 text-xs">{customer.createdBy?.fullName || "Global Engine"}</td>
                  <td className="py-3">
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                      customer.status === "Active" ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"
                    }`}>
                  ...
                  <td className="py-3 text-right">
                    <button
                      onClick={() => toggleAccountStatus(customer._id, customer.status || "Active")}
                      className="transition-colors text-slate-400 hover:text-white"
                    >
                      {(customer.status || "Active") === "Active" ? <ToggleRight size={20} className="inline text-green-400" /> : <ToggleLeft size={20} className="inline text-red-400" />}
                    </button>
                  </td>
                </tr>
              ))}
...
const VehicleCatalogView = () => {
  ...
  if (loading) return <div className="text-xs text-[#6b7280] animate-pulse">Streaming hardware ledger specs...</div>;
          ...
          <tbody className="divide-y divide-[#1e2238]/50 text-slate-300">
            {vehicles.map((v) => (
              <tr key={v._id} className="hover:bg-white/[0.01]">
                ...
                <td className="py-3 font-semibold text-white">{v.make} {v.model}</td>
                ...
                <td className="py-3 text-right text-[#8a8fbc]">{v.createdBy?.fullName || "Hidden Metadata"}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
const PolicyContractsView = () => {
  ...
  if (loading) return <div className="text-xs text-[#6b7280] animate-pulse">Pulling live coverage risk pipelines...</div>;

  return (
    <div className="bg-[#0d0f1d] border border-[#1e2238] rounded-2xl p-6 animate-fadeIn">
      <h3 className="flex items-center gap-2 mb-4 text-xs font-bold tracking-wider text-white uppercase">
        <FileText size={14} className="text-[#644aff]" /> Live Platform Underwriting Feed
      </h3>
      <div className="overflow-x-auto">
        <table className="w-full text-xs text-left border-collapse">
          <thead>
          ...
          <tbody className="divide-y divide-[#1e2238]/50 text-slate-300">
            {policies.map((p) => (
              <tr key={p._id} className="hover:bg-white/[0.01]">
                <td className="py-3 font-mono text-slate-400 text-[11px] max-w-[120px] truncate">{p._id}</td>
                <td className="py-3 font-semibold text-white">{p.customerId?.fullName || "Unlinked Client"}</td>
                <td className="py-3 font-mono text-[#8a8fbc]">{p.vehicleId?.registration || "Unknown Asset"}</td>
                <td className="py-3 font-bold text-emerald-400">£{p.premiumAmount?.toFixed(2)}</td>
                <td className="py-3 text-[11px] text-slate-400">
                  {new Date(p.startDate).toISOString().split('T')[0]} ({p.startTime}) ➔ {new Date(p.endDate).toISOString().split('T')[0]} ({p.endTime})
                </td>
                <td className="py-3 text-right">
...
export default function SuperAdminDashboard() {
  ...
  const { register, handleSubmit, reset } = useForm({ defaultValues: { role: "Sub Admin" } });
  ...
  const handleProvisionAgent = async (formData) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.message || "Credential layer error.");

      setRegSuccess(`Successfully provisioned Agent node for ${formData.fullName}`);
      reset({ fullName: "", email: "", password: "", role: "Sub Admin" });
      syncPlatformMetrics();
    } catch (err) {
      setRegError(err.message || "Communication failure with cluster registry.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col flex-1 max-h-screen min-w-0 overflow-y-auto">
        <header className="h-16 border-b border-[#1e2238] px-6 md:px-10 flex items-center justify-between sticky top-0 bg-[#060814]/90 backdrop-blur-md z-40 shrink-0">
          <div className="flex items-center gap-4">
            <h1 className="text-xs font-bold tracking-widest text-white uppercase md:text-sm">{getHeaderContextTitle()}</h1>
            {isSyncingTelemetry && <RefreshCw size={12} className="text-[#644aff] animate-spin shrink-0 hidden sm:block" />}
          </div>
        ...
        <main className="flex-1 w-full max-w-6xl p-6 pb-24 mx-auto space-y-8 md:p-10 lg:pb-10">
                ...
                <div className="lg:col-span-2 bg-[#0d0f1d] border border-[#1e2238] rounded-2xl p-6 space-y-6">
                  <div>
                    <h3 className="flex items-center gap-2 text-xs font-bold tracking-wider text-white uppercase">
                      <UserPlus size={14} className="text-[#644aff]" /> Provision Sub-Admin Node
                    </h3>
                    <p className="text-[11px] text-[#6b7280] mt-1">Executes a pipeline registration to append sub-tier operational administrators.</p>
                  </div>

                  <form onSubmit={handleSubmit(handleProvisionAgent)} className="space-y-4">
                    {regSuccess && <div className="flex items-center gap-2 p-3 text-xs font-semibold text-green-400 border rounded-xl bg-green-500/5 border-green-500/10"><CheckCircle size={14} /> {regSuccess}</div>}
                    {regError && <div className="flex items-center gap-2 p-3 text-xs font-semibold text-red-400 border rounded-xl bg-red-500/5 border-red-500/10"><AlertTriangle size={14} /> {regError}</div>}

                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-bold uppercase text-[#8a8fbc] tracking-wider">Agent Full Name</label>
                        <input type="text" required {...register("fullName")} placeholder="e.g. Sarah Jenkins" className="w-full bg-white/5 border border-[#1e2238] rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-[#644aff] transition-colors" />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-bold uppercase text-[#8a8fbc] tracking-wider">Corporate Access Email</label>
                        <input type="email" required {...register("email")} placeholder="name@cuvvaclone.com" className="w-full bg-white/5 border border-[#1e2238] rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-[#644aff] transition-colors" />
                      </div>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold uppercase text-[#8a8fbc] tracking-wider">Initial Account Password</label>
                      <input type="password" required {...register("password")} placeholder="••••••••" className="w-full bg-white/5 border border-[#1e2238] rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-[#644aff] transition-colors" />
                    </div>

                    <button type="submit" disabled={loading} className="w-full py-3 bg-[#644aff] hover:bg-[#523ad1] text-white font-bold rounded-xl text-[11px] uppercase tracking-wider transition-all disabled:opacity-40 active:scale-[0.99]">
                      {loading ? "Committing Database Records..." : "Authorize & Provision Credentials"}
                    </button>
                  </form>
                </div>

                <div className="bg-[#0d0f1d] border border-dashed border-[#1e2238] rounded-2xl p-6 text-xs text-[#8a8fbc] space-y-4">
                  <h4 className="font-bold text-white uppercase text-[10px] tracking-wider flex items-center gap-1.5 text-amber-400"><ShieldAlert size={12} /> Hierarchical Boundary Warnings</h4>
                  <p className="leading-relaxed">Sub-Admins provisioned through this entry vector receive structural database execution clearance roles.</p>
                  <ul className="list-disc pl-4 space-y-2 text-[#6b7280]">
                    ...
                    <li>Strictly forbidden from editing super-tier system keys.</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          {activeTab === "accounts" && <AccountManagementView onActionSuccess={syncPlatformMetrics} />}
